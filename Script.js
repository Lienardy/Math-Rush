...
const homeLeaderboard30 =
    document.getElementById("homeLeaderboard30");

const homeLeaderboard60 =
    document.getElementById("homeLeaderboard60");

const gameLeaderboardList =
    document.getElementById("gameLeaderboardList");

const gameLeaderboardTitle =
    document.getElementById("gameLeaderboardTitle");
...
async function getOnlineLeaderboard(gameMode) {
...
async function submitOnlineScore(playerNameValue, scoreValue, gameMode) {
    const { data, error } = await supabaseClient
        .from("leaderboard")
        .insert({
    ...
    leaderboardRealtimeChannel = supabaseClient
        ...
        .subscribe((status) => {
            ...
            if (status === "SUBSCRIBED") {
                console.log("Realtime leaderboard connected!");
            }

            if (status === "CHANNEL_ERROR") {
                console.error(
                    "Realtime leaderboard channel error."
                );
            }

            if (status === "TIMED_OUT") {
                console.error(
                    "Realtime leaderboard connection timed out."
                );
            }
        });
}
...
async function showHomeLeaderboards() {
    ...
    const scores30 =
        await getOnlineLeaderboard(30);

    const scores60 =
        await getOnlineLeaderboard(60);
    ...
    if (scores30.length === 0) {
        ...
        homeLeaderboard30.appendChild(li);
    } else {
        scores30
            .slice(0, 5)
            .forEach(player => {
    ...
    if (scores60.length === 0) {
        const li =
            document.createElement("li");

        li.textContent = "No scores yet";

        homeLeaderboard60.appendChild(li);
    } else {
        scores60
            .slice(0, 5)
            .forEach(player => {
...
function showVersusQuestion(question) {
    ...
    function checkAnswer(
        ...
        const buttons1 =
            player1Choices.querySelectorAll("button");

        const buttons2 =
            player2Choices.querySelectorAll("button");
